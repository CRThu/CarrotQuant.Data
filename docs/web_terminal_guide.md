...
- **独立格式数据范围表格 (`TableManagementGrid`)**：展开数据表后，独立显示 `Parquet` 与 `CSV` 各在物理磁盘上的数据范围起止时间、总记录行数与独立复选框。
- **右侧实时日志终端卡片 (`LogTerminal`)**：在数据管理中心页面右侧嵌入暗黑极客风日志视窗，基于 `/api/v1/logs/stream` SSE 接口实时推送全局 Loguru 系统与引擎日志，提供等级筛选 (Filter)、Auto-scroll 自动滚动切换、一键清空与复制功能。
