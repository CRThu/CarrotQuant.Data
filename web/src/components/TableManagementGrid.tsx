...
import { ChevronRight, ChevronDown, RefreshCw, ShieldAlert, CheckCircle2, Clock, Play } from 'lucide-react';
...
export const TableManagementGrid: React.FC<TableManagementGridProps> = ({
  ...
  const [selectedFormats, setSelectedFormats] = useState<Record<string, { parquet: boolean; csv: boolean }>>(() => {
    ...
    tables.forEach((t) => {
      init[t.table_id] = { parquet: true, csv: true };
    });
    return init;
  });
  ...
  const getTdxMode = (tableId: string) => tableTdxModes[tableId] || 'local';
  ...
  const handleStartSync = (targetTableIds?: string[]) => {
    ...
    targets.forEach((tid) => {
      const fmts = selectedFormats[tid] || { parquet: true, csv: true };
        ...
        <table className="w-full text-left text-xs text-slate-300">
          <thead className="bg-slate-950/80 text-slate-400 font-semibold border-b border-slate-800">
            <tr>
              ...
              <th className="py-3 px-3 min-w-[340px]">物理存储格式与水位线 (Parquet / CSV)</th>
              ...
              return (
                <React.Fragment key={item.table_id}>
                  ...
                  <tr className={`hover:bg-slate-800/40 transition-colors ${isSelected ? 'bg-cyan-950/10' : ''}`}>
                    ...
                    {/* 右侧：紧凑平铺展示 Parquet / CSV 状态、格式与起止日期 */}
                    <td className="py-2.5 px-3">
                      <div className="flex flex-col space-y-1.5 text-[10px]">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {pqMeta.exists ? (
                            <div className="flex items-center space-x-2 text-slate-300">
                              <span className="text-cyan-300">{pqMeta.total_bars.toLocaleString()} 条</span>
                            ...
                            <span className="text-slate-600 font-sans">(未落盘)</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {csvMeta.exists ? (
                            <div className="flex items-center space-x-2 text-slate-300">
                              <span className="text-amber-300">{csvMeta.total_bars.toLocaleString()} 条</span>
                              <span className="text-slate-600">|</span>
                              <span className="text-slate-400">
                    ...
                    <td className="py-2.5 px-3">
                      {statusObj?.status === 'running' ? (
                        ...
                        >
                          ...
                          <span className="font-sans truncate max-w-[160px]">{statusObj.message || '已完成'}</span>
                        </div>
                        ...
                        >
                          ...
                          <span className="font-sans truncate max-w-[140px]">{statusObj.message || statusObj.error_msg || '同步失败'}</span>
                        </div>
                      ) : (
