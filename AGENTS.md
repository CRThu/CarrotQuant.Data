...
| CSV 排序 | `["timestamp"]` (单 symbol 文件语义) | `["timestamp", "symbol"]` (Time-First) | 动态取前两列 + `["symbol"]` |
| Parquet 排序 | `["symbol", "timestamp"]` (Symbol-First，适配 C# MMF 索引) | `["timestamp", "symbol"]` (Time-First) | 动态取前两列 + `["symbol"]` |
| 列探测 | 必须包含 `symbol` 和 `timestamp` | 自动检查 `symbol` 列，缺失则仅按 `["timestamp"]` 排序 | 无 timestamp，动态检测排序键 |
