...
CarrotQuant.Data 是一个轻量级、模块化的本地金融数据同步与管理工具。它从免费数据源（Baostock）获取 A 股/指数数据，清洗后持久化到本地 CSV/Parquet 文件，供量化研究和回测使用。
...
**技术栈**：Python >= 3.12, Polars (数据处理), Baostock (数据源), FastAPI (API), Typer (CLI), Loguru (日志), Pydantic-Settings (配置)
...
- `get_provider(table_id)` → BaseProvider
  - 解析 table_id 末段 → 实例化对应 Provider (目前仅 baostock)
